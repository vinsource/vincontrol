...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div class="inner-wrap">
        ...
        <div id="data">
            <%= Html.Partial("FavoriteData", Model) %>
        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $('#Year').change(function () {
            $.ajax({
                type: "GET",
                dataType: "html",
                url: '/Ajax/Makes?year=' + $('#Year').val() + "&type=favorite",
                data: {},
                cache: false,
                traditional: true,
                success: function (result) {
                    $('#favorites_Make').html(result);
                    $('#favorites_Model').html('<select id="Model" name="Model" style="width:70px;"><option value="">Model</option></select>');
                    $('#favorites_Trim').html('<select id="Trim" name="Trim" style="width:70px;"><option value="">Trim</option></select>');
                    Filter();
                },
                error: function (err) {
        ...
        $('#Make').live('change', function () {
            $.ajax({
                type: "GET",
                dataType: "html",
                url: '/Ajax/Models?year=' + $('#Year').val() + '&make=' + $('#Make').val() + "&type=favorite",
                data: {},
                cache: false,
                traditional: true,
                success: function (result) {
                    $('#favorites_Model').html(result);
                    $('#favorites_Trim').html('<select id="Trim" name="Trim" style="width:70px;"><option value="">Trim</option></select>');
                    Filter();
                },
                error: function (err) {
        ...
        $('#Model').live('change', function () {
            $.ajax({
                type: "GET",
                dataType: "html",
                url: '/Ajax/Trims?year=' + +$('#Year').val() + '&make=' + $('#Make').val() + '&model=' + $('#Model').val() + "&type=favorite",
                data: {},
                cache: false,
                traditional: true,
                success: function (result) {
                    $('#favorites_Trim').html(result);
                    Filter();
                },
                error: function (err) {
    ...
    function Filter() {
        ...
        $.ajax({
            type: "GET",
            dataType: "html",
            url: '/Auction/FilterFavoriteVehicles?year=' + $('#Year').val() + "&make=" + $('#Make').val() + "&model=" + $('#Model').val() + "&trim=" + $('#Trim').val() + "&lane=" + $('#Lane').val() + "&run=" + $('#Run').val() + "&seller=" + $('#Seller').val() + "&category=" + $('#Category').val(),
            data: {},
            cache: false,
            traditional: true,
            success: function (result) {
