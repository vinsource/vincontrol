        ...
        <div class="scrollingArea">
            <table id="tblVehicles_upcoming" class="list" cellspacing="0">
                <thead>
                    ...
                    <% foreach (var group in Model.AuctionList.Where(i => !i.IsSold).OrderBy(i => i.RegionName).GroupBy(i => i.RegionName))
                    {%>
                    <tbody class="tablesorter-no-sort">
                    <tr id="auction_<%= group.Key %>">
                        <td colspan="12"><b><%= group.Key %></b> &nbsp;&nbsp; <i>(<%= group.Count() > 1 ? group.Count() + " vehicles" : "1 vehicle" %>)</i></td>
                    </tr>
                    </tbody>    
                        ...
                        <tr id="date_<%= group.Key %>" style="display:none;">
                            <td colspan="12"><i><%= subgroup.Key.ToString("MM/dd/yyyy") %></i></td>
                        </tr>
                        </tbody>
                        <tbody>
            ...
            <table id="tblVehicles_past" class="list" cellspacing="0" style="display:none;">
                <thead>
                    ...
                    <tr id="auction_<%= group.Key %>">
                        ...
                        <tbody>
                        <% foreach (var auction in subgroup.OrderByDescending(i => i.Year).OrderBy(i => i.Seller))
                        {%>
                        <tr id="viewByAuction_<%= group.Key %>_<%= auction.Id %>" style="display:none;">
                            <td class="fav" width="20">
                                <input id="chkFavorite_<%= auction.Id %>_<%= auction.Vin %>" type="checkbox" <%= auction.IsFavorite ? "checked" : "" %>/>
        ...
        $("input[id^='chkFavorite_']").click(function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    window.location.href = '<%= Url.Action("Favorites","Auction") %>';
                },
                error: function (err) {
