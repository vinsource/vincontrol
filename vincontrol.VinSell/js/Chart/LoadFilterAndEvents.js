...
function AssignEvents() {
    ...
    $('#btnPrintBucketJump').click(function () {
        ...
        if ($selectedCar.listingid == undefined)
            ...
            url += "&inventoryType=" + inventorytype.value;
    ...
    function openCarFaxReport(vin, popupWidth, popupHeight) {
        ...
        //window.open('/Chart/CarFaxReportFromAutoTrader?vin=' + vin, 'CarFax Report', 'location=no,toolbar=no,directories=no,status=no,menubar=no,scrollbars=no,width=' + popupWidth + ',height=' + popupHeight + ',top=' + top + ',left=' + left);
    }
    ...
    $('a[id^=checkCarFax_]').live('click', function () {
        ...
        //$.post('/Chart/CarFaxReportFromAutoTrader") %>', { vin: vin }, function (data) {
        //    //if (data == "SessionTimeOut") {
        //    //    parent.$.fancybox.close();
        //    //    ShowWarningMessage("Your session is timed out. Please login back again");
        //    //    var actionUrl = '<%= Url.Action("LogOff", "Account" ) %>';
