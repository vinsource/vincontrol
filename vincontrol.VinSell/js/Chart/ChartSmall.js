...
function UpdateCarRanking(isCarscom, vinCurrentScreen) {
    ...
    if ($currentFilterredList != undefined && $currentFilterredList.length > 0) {
        ...
        if (numberOfCars > 0) {
            ...
            var updateRankingUrl = "/Chart/UpdateCarRanking?listingId=" + listingId + "&carRanking=" + carRanking + "&numberOfCars=" + numberOfCars + "&oldCarRanking=" + $dCar.ranking + "&oldNumberOfCars=" + $dCar.carsOnMarket + "&isCarscom=" + isCarscom + "&smallestPrice=" + smallestPrice + "&averagePrice=" + averagePrice + "&largestPrice=" + largestPrice + "&screen=" + vinCurrentScreen;
            $.ajax({
                type: "POST",
