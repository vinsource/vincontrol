...
namespace vincontrol.VinSocial.YoutubeHelper
{
    ...
    public class LoopbackServerAuthorizationFlow : INativeAuthorizationFlow
    {
        ...
        private const string LoopbackServerHtmlResponse =
            "<html><head><title>{APP} - OAuth Authentication</title></head><body><h1>Authorization for {APP}</h1><p>The application has received your response. You can close this window now.</p><script type='text/javascript'>window.setTimeout(function() { window.open('', '_self', ''); window.close(); }, 100);if (window.opener) { window.opener.checkToken(); }</script></body></html>";
        ...
        private string HandleRequest(HttpListenerContext context)
        {
            ...
            finally
            {
                ...
                {
                    string response = LoopbackServerHtmlResponse.Replace("{APP}", Util.ApplicationName);
