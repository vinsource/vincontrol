...
namespace vincontrol.Helper
{
    public sealed class CarFaxHelperBAK
    {
        ...
        public static CarFaxViewModel GetCarFaxReportInDatabase(string vin)
        {
            ...
            var  carFaxTmp = LinqHelper.CheckVinHasCarFaxReport(vin);

            if (carFaxTmp != null)
            {
                try
                {
                    carfax.Vin = carFaxTmp.Vin;
                    carfax.NumberofOwners = carFaxTmp.Owner.ToString();
                    carfax.ServiceRecords = carFaxTmp.ServiceRecord.GetValueOrDefault();
                    carfax.AccidentCounts = carFaxTmp.Accident.GetValueOrDefault();
                    carfax.PriorRental = carFaxTmp.PriorRental.GetValueOrDefault();

                    var stringList = carFaxTmp.WindowSticker.Split(new String[] { "," },
        ...
        //                        var cars = context.Inventories.Where(i => i.Vehicle.Vin.Equals(vin));
