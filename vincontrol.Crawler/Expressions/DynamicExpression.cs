...
namespace vincontrol.Crawler.Expressions
{
    public class DynamicExpression
    {
        ...
        public static string EvaluateWithStringResult(string expression)
        {
        ...
        public static bool EvaluateWithBoolResult(string expression)
        {
            var code = string.Format  // Note: Use "{{" to denote a single "{"  
            (
                "public static class Func{{ public static bool func(){{ return {0};}}}}",
                expression
            );
