    ...
    public class SDKHandler : IDisposable
    {
        ...
        public Bitmap DownloadImage(IntPtr ObjectPointer)
        {
            lock (LVlock)
            {
                ...
                var extension = Path.GetExtension(dirInfo.szFileName);
                if (extension != null)
                {
                    ...
                    if (ext == ".jpg" || ext == ".jpeg")
                    {
                        lock (LVlock)
                        {
                            ...
                            unsafe
                            {
                                ...
                                using (var ums = new UnmanagedMemoryStream((byte*)jpgPointer.ToPointer(), length, length, FileAccess.Read))
                                {
        ...
        private Bitmap GetImage(IntPtr imgStream, EDSDK.EdsImageSource imageSource)
        {
            ...
            try
            {
                ...
                Error = EDSDK.EdsCreateImageRef(imgStream, out img_ref);
            ...
            {
                //Release all data
                if (imgStream != IntPtr.Zero) Error = EDSDK.EdsRelease(imgStream);
