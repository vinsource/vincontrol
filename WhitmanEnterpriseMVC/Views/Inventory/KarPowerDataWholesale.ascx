...
<div id="kbb-row" class="mr-row" <%= data.IsSelected ? "" : "style='display:none'" %>>
    <div class="range-item label">
        <a id="kbbRow_<%= data.SelectedTrimId %>" class="iframe" target="_blank" style="font-size: .7em" title="<%=data.SelectedTrimName%>" href="<%= Url.Action("GetSingleKarPowerSummaryForWholesale", "Market", new { listingId = ViewData["LISTINGID"], trimId = data.SelectedTrimId }) %>">
