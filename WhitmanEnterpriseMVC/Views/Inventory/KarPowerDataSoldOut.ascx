...
<div id="kbb-row" class="mr-row" <%= data.IsSelected ? "" : "style='display:none'" %>>
    <div class="range-item label">
        <a id="kbbRow_<%= data.SelectedTrimId %>" class="iframe" target="_blank" style="font-size: .7em" title="<%=data.SelectedTrimName%>" href="<%= Url.Action("GetSingleKarPowerSummaryForSoldCars", "Market", new { listingId = ViewData["LISTINGID"], trimId = data.SelectedTrimId }) %>">
...
<%}%>
<div>
    <a id="aLoadFullKBBTrims" style="float: left; font-weight: normal; font-size: .9em;" href="javascript:;">Not a correct trim? Click here</a>
    <br />
  <%--  <a id="kbbRow" class="iframe" target="_blank" style="float: right; font-weight: normal; font-size: .9em;"
        href="<%= Url.Action("GetKarPowerSummary", "Market", new { listingId = (string)ViewData["LISTINGID"] }) %>">
        View KBB Summary Report</a>--%>
    
    <br />
    <div><font size="1">* Total Value = Base WholeSale +/- Mileage Adjustment</font></div>
</div>
