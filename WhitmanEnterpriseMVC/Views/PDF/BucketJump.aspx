<%@ Page Language="C#" Inherits="System.Web.Mvc.ViewPage<WhitmanEnterpriseMVC.Models.BucketJumpViewModel>" %>
        ...
        <div class="graph-title-bar" style="margin-bottom: 30px; margin-top: 320px; text-align:center;">
            ...
            <div id="printable-list" style="padding:0;margin:0 auto;display: block;">
               ...
               foreach (var item in Model.ChartGraph.ChartModels){%>
                                <tr <%= item.IsTargetCar ? "class=isTargetCar" : "" %>>
                                    <td><%=i++%></td>
                                    <td><%= Model.CarOnMarket.Year %></td>
                                    <td><%= Model.CarOnMarket.Make %></td>
                                    <td><%= Model.CarOnMarket.Model %></td>
                                    <td><%= item.Trims.Count > 0 ? item.Trims.FirstOrDefault() : "" %></td>
                                    <td><%=item.Distance%></td>
                                    <td><%=item.Certified ? "Yes" : "No"%></td>
                                    <td><%=item.Seller != null ? item.Seller.SellerName : ""%></td>
                                    <td><b><%=item.Miles.ToString("0,0")%></b></td>
                                    <td><b><%=item.Price.ToString("c0")%></b></td>
                                    <td><%=item.CarsCom ? "Yes" : "No"%></td>
                                    <td><%=item.AutoTrader ? "Yes" : "No"%></td>
                                </tr>
                            <%}%>                            
                        </tbody>
                    </table>
