...
//                if (arr[i].replace(/^\s+|\s+$/g, '').replace(/\s\s+/g, ' ') == obj.replace(/^\s+|\s+$/g, '').replace(/\s\s+/g, ' ')) return true;
        ...
        $(document).ready(function () {
            ...
            var trimValue = $("#SelectedTrimItem").val();
            if (trimValue) {
            ...
            $("#SelectedTrimItem").live('change', function () {
                ...
                if (name != '') {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "/Appraisal/GetVehicleInformationFromStyleId?appraisalId=" + $('#AppraisalGenerateId').val() + "&vin=" + $('#VinNumber').val() + "&styleId=" + id + "&isTruck=False&styleName=" + style + "&cusStyle=" + $("#CusTrim").val(),
                        data: {},
