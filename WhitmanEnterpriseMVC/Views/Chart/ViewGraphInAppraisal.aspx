...
<% } %>
    <div id="result">
        <h2>
            <%=ViewData["CarTitle"]%>  <span id="PriceChangeInAppraisal"></span>
        </h2>
       
        
         <div>
                  <input type="radio" id="radioAutoTrader" checked="checked" name="webSource" />AutoTrader
                <input type="radio" id="radioCarsCom" name="webSource" />CarsCom
                ...
                <a id="seeNationwideMarketData" href="javascript:;">Click here to see nationwide market data</a>
            </div>
        <div id="rangeNav">
            <span class="selected" id="100">100</span>
            <span id="nation">State</span>
        </div>
        <div id="filter-wrap">
           <%-- Options
            ...
            <input type="radio" id="all" checked="checked" name="dealertype" />All
            <input type="radio" id="franchise" name="dealertype" />Franchise
            <input type="radio" id="independant" name="dealertype" />Independent
        ...
        <div class="market-info">
			<h4 style="margin-bottom: 0;">Market Price Ranges</h4>
			<h5 style="margin-top: 0;margin-bottom:.5em;">
				<span class="blue" id="low"></span>|<span class="green" id="middle"></span>|
    ...
    <script src="<%=Url.Content("~/js/Chart/ViewGraphInAppraisal.js")%>" type="text/javascript"></script>
    ...
    <script type="text/javascript">
        ...
        var requestCarsComUrl = '<%= Url.Action("GetMarketDataByAppraisalFromCarsComWithHttpPost", "chart", new { ListingId = "PLACEHOLDER" } ) %>';
        var requestAutoTraderUrl = '<%= Url.Action("GetMarketDataByAppraisalFromAutoTraderWithHttpPost", "chart", new { ListingId = "PLACEHOLDER" } ) %>';
        var requestHiddenCarsComUrl = '<%= Url.Action("LoadMarketDataByAppraisalFromCarsComIntoSession", "chart", new { ListingId = "PLACEHOLDER" } ) %>';
        var requestHiddenAutoTraderUrl = '<%= Url.Action("LoadMarketDataByAppraisalFromAutoTraderIntoSession", "chart", new { ListingId = "PLACEHOLDER" } ) %>';
        var updateSalePrice = '<%= Url.Content("~/Inventory/UpdateSalePrice") %>';
        var logOff = '<%= Url.Action("LogOff", "Account" ) %>';
        var detailUrl = '<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new { AppraisalId = "PLACEHOLDER" } ) %>';
        var waitingImage = '<%= Url.Content("~/images/ajax-loader1.gif") %>';
        var chartType = 'Appraisal';
