...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var TruckType = $("#SelectedTruckType");

            TruckType.live('change', function () {

                $('#elementID').removeClass('hideLoader');

                var type = TruckType.val();

                $.post('<%= Url.Content("~/Ajax/TruckCategoryAjaxPost") %>', { TruckType: type }, function (data) {

                    var exitems = "";

                    $.each(data, function (i, data) {
                        var text = data.toString();
                        exitems += "<option value='" + text + "'>" + text + "</option>";
                    });

                    $("#SelectedTruckCategory").html(exitems);
            ...
            $("#SelectedTrim").live('change', function () {
                ...
                if (name != '') {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "/Decode/GetVehicleInformationFromStyleId?vin=" + $('#VinNumber').val() + "&styleId=" + id + "&isTruck=True&styleName=" + style + "&cusStyle=" + $("#CusTrim").val(),
                        data: {},
