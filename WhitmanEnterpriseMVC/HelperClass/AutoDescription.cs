...
namespace WhitmanEnterpriseMVC.HelperClass
{
    ...
    public class AutoDescription
    {
        ...
        public whitmanenterprisewarehouseEntities Context
        {
            get { return _context; }
        }
        ...
        public bool AssignAutoDescription(int newListingId)
        {
            bool isSuccess = false;
            var newInventory = _context.whitmanenterprisedealershipinventories.FirstOrDefault(x => x.ListingID == newListingId);
            if (newInventory!=null && AllowAutoDescription(newInventory.DealershipId.GetValueOrDefault()))
            {
                ...
                if (newInventory != null && (newInventory.EnableAutoDescription == null || (newInventory.EnableAutoDescription.Value)))
                {
                    ...
                    var template = newInventory.Template.HasValue ? GetTemplate(newInventory.Template.Value) : GetRandomizedTemplate(inventoryStatus);
                    if (template == null) throw new ArgumentNullException("template");
                    var setting = _context.whitmanenterprisesettings.FirstOrDefault(i => i.DealershipId == newInventory.DealershipId);
                    var disclaimer =
              _context.vincontrolrebates.Where(
                  i =>
                  (i.DealerId == newInventory.DealershipId) && (i.Year == newInventory.ModelYear) && (i.Make == newInventory.Make) && (i.Model == newInventory.Model) && (i.Trim == newInventory.Trim)).Select(j => j.Disclaimer).FirstOrDefault();
                    ...
                    var autoDescriptionViewModel = new AutoDescriptionViewModel()
                    {
                        ...
                        AdditionalOptions = newInventory.CarsOptions,
                        ...
                        Transmission = String.IsNullOrEmpty(newInventory.Tranmission) ? "" : newInventory.Tranmission,
                    ...
                    newInventory.Template = template.Id;
                    newInventory.Descriptions = GenerateAutoDescription(template.Content, autoDescriptionViewModel);
                    isSuccess = true;
        ...
        private TemplateParser CreateTemplateParser(AutoDescriptionViewModel viewModel)
        {
            ...
            if (viewModel.PriorRental)
                templateParser.AddTag(Token.RentalUnwindDealerDemoBrandedTitle, "Prior Rental.");


            if (viewModel.DealerDemo)
                templateParser.AddTag(Token.RentalUnwindDealerDemoBrandedTitle, "Dealer Demo.");


            if (viewModel.Unwind)
                templateParser.AddTag(Token.RentalUnwindDealerDemoBrandedTitle, "Unwind.");


            if (viewModel.BrandedTitle)
                templateParser.AddTag(Token.RentalUnwindDealerDemoBrandedTitle, "Branded Title.");
            ...
            templateParser.AddTag(Token.CertifiedBy, viewModel.Certified ? "Certified by " + viewModel.Make + "." : string.Empty);
            templateParser.AddTag(Token.NewArrival, viewModel.DaysInInventory <= 30 ? "NEW ARRIVAL!!" : string.Empty);
            templateParser.AddTag(Token.Address, String.IsNullOrEmpty(viewModel.Address) ? Token.Address : viewModel.Address);
            templateParser.AddTag(Token.Phone, String.IsNullOrEmpty(viewModel.Phone) ? Token.Phone : viewModel.Phone);
            templateParser.AddTag(Token.PopularColor, _popularColors.Contains(viewModel.Color) ? viewModel.Color : string.Empty);
            ...
            if (!String.IsNullOrEmpty(viewModel.MSRP))
            {
                templateParser.AddTag(Token.MSRP, viewModel.MSRP);
            ...
            if (!String.IsNullOrEmpty(viewModel.Packages))
            {
                ...
                {
                    templateParser.AddTag(Token.AdditionalOptions, " as well as the following Options: " + viewModel.AdditionalOptions);
                }
                else
                {
            ...
            else
            {
                if (!String.IsNullOrEmpty(viewModel.AdditionalOptions))
                {
                    ...
                    templateParser.AddTag(Token.AdditionalOptions, "the following Options: " + viewModel.AdditionalOptions);
                }
                else
                {
                    templateParser.AddTag(string.Empty);
            ...
            if (viewModel.CarsOnMarket < 30)
            {
                ...
                {
                    templateParser.AddTag(Token.FuelEfficient, " and is very fuel efficient, getting " + viewModel.FuelHighway + " miles per gallon");
                }
                else
                {
            ...
            if (DateTime.Now.Year > viewModel.Year)
            {
                templateParser.AddTag(Token.LowMileage,
                                      viewModel.Mileage / (DateTime.Now.Year - viewModel.Year) <= 12000 ? "It also has low mileage."
                                          : string.Empty);
            }
            else
            {
            ...
            templateParser.AddTag(Token.BeginningKindOfSentence, GenerateAutoDescriptionForSentences(GetRandomizedSentence(1, viewModel.DealershipName), viewModel));
            templateParser.AddTag(Token.NearTheEndSentence, GenerateAutoDescriptionForSentences(GetRandomizedSentence(2, viewModel.DealershipName), viewModel));

            if(viewModel.DealerId!=1660)
                templateParser.AddTag(Token.VeryLastSentence, GetVeryLastSentence(String.Format("http://vincontrol@DOTcom/tradeinvalue/{0}", viewModel.DealershipName.ToLower().Replace(" ", string.Empty))));
    ...
    public class Token
    {
        ...
        public const string RentalUnwindDealerDemoBrandedTitle = "[RentalUnwindDealerDemoBrandedTitle]";
