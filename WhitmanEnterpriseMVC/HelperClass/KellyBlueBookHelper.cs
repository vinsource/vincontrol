...
{
    public sealed class KellyBlueBookHelper
    {
        ...
        public static KellyBlueBookViewModel GetDirectFullReport(string Vin, string ZipCode, string Mileage)
        {
            var m_serviceClient = new VehicleInformationService2008R2Client();
                            ...
                            if (!String.IsNullOrEmpty(vehicleConfig.Trim.Value))
                            {
                                ...
                                foreach (Valuation va in vehicleValue.ValuationPrices)
                                {
                                    ...
                                    if (va.PriceType == PriceType.TradeInFair)
                                        TradeInDetail.TradeInFairPrice = va.Value.ToString("c0");
                                    else if (va.PriceType == PriceType.TradeInGood)

                                        TradeInDetail.TradeInGoodPrice = va.Value.ToString("c0");

                                    else if (va.PriceType == PriceType.TradeInVeryGood)

                                        TradeInDetail.TradeInVeryGoodPrice = va.Value.ToString("c0");
                                    else if (va.PriceType == PriceType.TradeInExcellent)

                                        TradeInDetail.TradeInExcellentPrice = va.Value.ToString("c0");
                                    else if (va.PriceType == PriceType.Wholesale)

                                        trimReport.WholeSale = va.Value.ToString("c0");
                                    ...
                                    else if (va.PriceType == PriceType.BaseWholesale)
                                    {
                                        ...
                                        trimReport.BaseWholesale =
                                            (wholeSale - trimReport.MileageAdjustment).ToString("c0");
        ...
        public static KellyBlueBookViewModel GetDirectFullReport(string Vin, string ZipCode, string Mileage, int TrimId)
        {
            var m_serviceClient = new VehicleInformationService2008R2Client();
                        ...
                        foreach (var vehicleConfig in vehicleConfigArray)
                        {
                            ...
                            foreach (Valuation va in vehicleValue.ValuationPrices)
                            {
                                ...
                                if (va.PriceType == PriceType.TradeInFair)
                                    ...
                                    trimReport.WholeSale = (va.Value + wholeSaleAdjustment).ToString("c0");
                                ...
                                else if (va.PriceType == PriceType.BaseWholesale)
                                {
                                    ...
                                    trimReport.BaseWholesale =
                                        (wholeSale - trimReport.MileageAdjustment + wholeSaleAdjustment).ToString("c0");
                        ...
                        foreach (var vehicleConfig in vehicleConfigArray)
                        {
                            ...
                            if (!String.IsNullOrEmpty(vehicleConfig.Trim.Value) && vehicleConfig.Trim.Id == TrimId)
                            {
                                ...
                                foreach (Valuation va in vehicleValue.ValuationPrices)
                                {
                                    ...
                                    if (va.PriceType == PriceType.TradeInFair)
                                        TradeInDetail.TradeInFairPrice = (va.Value + wholeSaleAdjustment).ToString("c0");
                                    else if (va.PriceType == PriceType.TradeInGood)

                                        TradeInDetail.TradeInGoodPrice = (va.Value + wholeSaleAdjustment).ToString("c0");

                                    else if (va.PriceType == PriceType.TradeInVeryGood)

                                        TradeInDetail.TradeInVeryGoodPrice =
                                            (va.Value + wholeSaleAdjustment).ToString("c0");
                                    else if (va.PriceType == PriceType.TradeInExcellent)
                                    ...
                                    else if (va.PriceType == PriceType.BaseWholesale)
                                    {
                                        ...
                                        trimReport.BaseWholesale =
                                            (wholeSale - trimReport.MileageAdjustment + wholeSaleAdjustment).ToString(
                                                "c0");
        ...
        public static KellyBlueBookViewModel GetFullReport(string Vin, string ZipCode, string Mileage)
        {
        ...
        public static KellyBlueBookViewModel GetFullReportWithSavingChanges(string Vin, string ZipCode, string Mileage)
        {
        ...
        public static KellyBlueBookViewModel GetKBBModelInDatabase(string Vin)
        {
            ...
            if (status == 1)
            {
                ...
                var list = context.whitmanenterprisekbbs.Where(x => x.Vin.Equals(Vin));

                foreach (whitmanenterprisekbb kbb in list)
                {
                    ...
                    var trimReport = new KellyBlueBookTrimReport()
                    {
                        ...
                        BaseWholesale = CommonHelper.ConvertToCurrency(kbb.BaseWholeSale),
                        MileageAdjustment = CommonHelper.ConvertStringToInterger(kbb.MileageAdjustment),
                        WholeSale = CommonHelper.ConvertToCurrency(kbb.WholeSale),
                        TrimName = kbb.Trim,
        ...
        public static KellyBlueBookViewModel GetKBBModelInDatabase(string Vin, int TrimId)
        {
            ...
            if (status == 1)
            {
                ...
                var list = context.whitmanenterprisekbbs.Where(x => x.Vin.Equals(Vin) && x.TrimId == TrimId);
        ...
        public static TradeInVehicleModel GetKBBTradeInValue(int VehicleId, int Mileage, string SelectedOptions, string ZipCode, bool setPrice, decimal variance)
        {
            ...
            {
                returnVehicle.TradeInFairPrice = "NA";
                returnVehicle.TradeInGoodPrice = "NA";
            }
        ...
        {
            var tradeInFair = vehicleValue.ValuationPrices.Where(i => i.PriceType == PriceType.TradeInFair).FirstOrDefault();
            var tradeInGood = vehicleValue.ValuationPrices.Where(i => i.PriceType == PriceType.TradeInGood).FirstOrDefault();
            decimal calculatedGoodPrice = tradeInGood.Value + returnVehicle.SelectedOptionAdjustment;
            returnVehicle.TradeInGoodPrice = (tradeInGood != null && tradeInGood.Value > 0) ? calculatedGoodPrice.ToString("c0") : "NA";

            if (!setPrice)
            {
                returnVehicle.TradeInFairPrice = (tradeInFair != null && tradeInFair.Value > 0) ? (tradeInFair.Value + returnVehicle.SelectedOptionAdjustment).ToString("c0") : "NA";
            }
            else
            {
        ...
        private static string GetTradeInFairPrice(TradeInVehicleModel returnVehicle, Valuation tradeInFair, decimal calculatedGoodPrice, decimal variance)
        {
            if ((tradeInFair != null && tradeInFair.Value > 0))
            {
                if (returnVehicle.TradeInGoodPrice.Equals("NA"))
                {
                    return (tradeInFair.Value + returnVehicle.SelectedOptionAdjustment).ToString("c0");
                }
                else
                {
                    if (tradeInFair.Value + returnVehicle.SelectedOptionAdjustment + variance > calculatedGoodPrice)
                    {
                        return (calculatedGoodPrice).ToString("c0");
                    }
                    else
                    {
                        return (tradeInFair.Value + returnVehicle.SelectedOptionAdjustment + variance).ToString("c0");
        ...
        public static string BuildKBBReportInHtml(int listingId, string kbbVehicleId, int trimId, DealershipViewModel dealer, int printOption)
        {
            ...
            var trimDetail = GetVehicleSpecificationFromKBB(kbbVehicleId, dealer, row.Mileage, row.KBBOptionsId, printOption);
            ...
            if (String.IsNullOrEmpty(row.StockNumber))

                builder.AppendLine("<td align=\"right\"><b>Stock #N/A<br />" + String.Format("{0:MMM d, yyyy}", DateTime.Now) + "</b></td>");
            else
                builder.AppendLine("<td align=\"right\"><b>Stock " + row.StockNumber + "<br />" + String.Format("{0:MMM d, yyyy}", DateTime.Now) + "</b></td>");
            ...
            builder.AppendLine("<td align=\"center\" colspan=\"3\"><b>VIN: " + row.VINNumber + "</b></td>");
            ...
            builder.AppendLine("<td align=\"center\" colspan=\"2\"><b>Fuel City/Hwy " + row.FuelEconomyCity + "/" + row.FuelEconomyHighWay + " MPG</b></td><br />");
            ...
            builder.AppendLine("<b align=\"center\">Mileage Adjustment(" + CommonHelper.FormatNumberInThousand(row.Mileage) + ") miles................</b><br /><br />");
            ...
            if (dealer.OverrideDealerKbbReport)
            {
                var dealerGroupName =
                    context.whitmanenterprisedealergroups.First(x => x.DealerGroupId == dealer.DealerGroupId).
                        DealerGroupName;
        ...
        private static KellyBlueBookTrimReport GetVehicleSpecificationFromKBB(string KBBVehicleId, DealershipViewModel dealer, string Mileage, string SavedOption, int PrintOption)
        {
            var m_serviceClient = new VehicleInformationService2008R2Client();
            ...
            bool flag = Int32.TryParse(Mileage, out Mile);

            vehicleConfig.Mileage = Mile;
