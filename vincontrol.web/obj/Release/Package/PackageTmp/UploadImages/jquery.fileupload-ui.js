    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            add: function(e, data) {
                ...
                var isIE = that.getInternetExplorerVersion();
                // we can use ActiveXObject to get size of uploaded file in IE, 
                // but we need to change in the Internet Option manually
                //var myFSO = new ActiveXObject("Scripting.FileSystemObject");

                if ((data.files[0].size <= 1048576 && isIE == -1) || isIE > -1) {

                    that._adjustMaxNumberOfFiles(-data.files.length);
                    data.isAdjusted = true;
                    data.isValidated = that._validate(data.files);
                    data.context = that._renderUpload(data.files)
                    .appendTo($(this).find('.files')).hide(function() {
                    ...
                    countImages++;
                    $("#number").html("\n" + "0 out of " + countImages + " Pictures Uploaded");
                }
                else {
                    alert(data.files[0].name + " is in excess of the 1MB file size limit");
                    return false;
                }
            },
            ...
            // Callback for successful uploads:
                ...
                var that = $(this).data('fileupload');
                $("#number").html("\n" + index + " out of " + countImages + " Pictures Uploaded");
                index++;
                if (data.context) {
