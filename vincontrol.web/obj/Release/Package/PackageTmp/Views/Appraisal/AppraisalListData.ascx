...
<script type="text/javascript">
    $(document).ready(function () {
        $(".sForm").numeric({ decimal: false, negative: false }, function () { ShowWarningMessage("Positive integers only"); this.value = ""; this.focus(); });
        $(".appraisals_number_below").html('<%=Model.NumberOfRecords %>');    
        $(".SalePriceForm").validationEngine({ promptPosition: "bottomRight", scroll: false });
    });
...
<%--<div class="vin_listVehicle_holder">--%>
    ...
    %>
        ...
        <div class="contain_list">
            ...
            <div class="right_content_items right_content_items_img">
                <a class="vin_editProfile" href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                </a>
                <a href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                    <img alt="" height="61" src="<%= tmp.FirstPhoto %>" width="65"></a>
            </div>
            <a class="vin_editProfile" href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                <div class="right_content_items right_content_items_vin nav_long" title="<%=tmp.VinNumber %>">
                    <%= tmp.VinNumber.Length < 8 ? tmp.VinNumber : tmp.VinNumber.Substring(tmp.VinNumber.Length - 8, 8) %>
                </div>
            </a><a class="vin_editProfile" href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                <div class="right_content_items nav_middle">
                    <%= tmp.ModelYear %>
                </div>
            </a><a class="vin_editProfile" href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                <div class="right_content_items nav_long" title="<%=tmp.Make %>">
                    <%--<%= !String.IsNullOrEmpty(tmp.Make) && tmp.Make.Length > 8 ? tmp.Make.Substring(0, 8) + "..." : tmp.Make%>--%>
                    <%=tmp.Make %>
                </div>
            </a><a class="vin_editProfile" href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                <div class="right_content_items nav_long" title="<%=tmp.AppraisalModel %>">
                    <%--<%= !String.IsNullOrEmpty(tmp.AppraisalModel) && tmp.AppraisalModel.Length > 7 ? tmp.AppraisalModel.Substring(0, 7) + "..." : tmp.AppraisalModel%>--%>
                    <%=tmp.Model %>
                </div>
            </a><a class="vin_editProfile" href="<%= Url.Action("ViewProfileForAppraisal", "Appraisal", new {AppraisalId = tmp.AppraisalID}) %>">
                <div class="right_content_items nav_long" title="<%=tmp.Trim %>">
                    <%--<%= !String.IsNullOrEmpty(tmp.Trim) && tmp.Trim.Length > 7 ? tmp.Trim.Substring(0, 7) + "..." : tmp.Trim%>--%>
                    <%=tmp.Trim %>
                </div>
            </a>
            <div class="right_content_items nav_middle" title="<%=tmp.ExteriorColor %>">
            ...
            <div class="right_content_items right_content_items_appraiser nav_long" title="<%= tmp.AppraisalById %>">
                <%= tmp.ShortAppraisalBy %>
            </div>
            <form name="SaveSalePriceForm" class="SalePriceForm">
                <div class="right_content_items vehicle_price_appraisal divValue">
                    <input type="text" id="<%= tmp.AppraisalID %>" name="odometer" class="sForm" 
                        value="<%=tmp.Mileage.ToString("#,##0")%>" data-validation-engine="validate[funcCall[checkMiles]]">
                    <img class="imgLoadingAppraisal" src="../Content/images/ajaxloadingindicator.gif" height="15px"/>
                </div>
                <div class="right_content_items vehicle_price_appraisal divValue">
                    <input type="text" id="<%= tmp.AppraisalID %>" name="Acv" class="sForm" 
                        value="<%=tmp.ACV.HasValue ? tmp.ACV.Value.ToString("#,##0") : "" %>" data-validation-engine="validate[funcCall[checkSalePrice]]">
                    <img class="imgLoadingAppraisal" src="../Content/images/ajaxloadingindicator.gif" height="15px"/>
                </div>
            </form>
