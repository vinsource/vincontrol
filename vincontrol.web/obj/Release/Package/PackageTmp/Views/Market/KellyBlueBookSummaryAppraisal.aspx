<%@ Page Title="" Language="C#" Inherits="System.Web.Mvc.ViewPage<Vincontrol.Web.Models.KellyBlueBookViewModel>" %>
...
<%=Html.HiddenFor(x=>x.TrimReportList.First().MileageAdjustment) %>
 <% Html.EndForm(); %>
    ...
    <tr>
       ...
       <% foreach (var trimReport in Model.TrimReportList)
          { %>
           ...
           <tr>
               <td class="title trim"> <a onclick="javascript: $('#elementID').removeClass('hideLoader');" href="<%=Url.Content("~/Market/GetKellyBlueBookSummaryAppraisalByTrim?AppraisalId=") %><%=Model.AppraisalId %>&TrimId=<%=trimReport.TrimId %>"><%= ViewData["CarTitle"] %></a></td>
               <td class="title trim"> <a onclick="javascript: $('#elementID').removeClass('hideLoader');" href="<%=Url.Content("~/Market/GetKellyBlueBookSummaryAppraisalByTrim?AppraisalId=") %><%=Model.AppraisalId %>&TrimId=<%=trimReport.TrimId %>"><%= trimReport.TrimName %></a></td>
               <td class="title trim"><a onclick="javascript: $('#elementID').removeClass('hideLoader');" href="<%=Url.Content("~/Market/GetKellyBlueBookSummaryAppraisalByTrim?AppraisalId=") %><%=Model.AppraisalId %>&TrimId=<%=trimReport.TrimId %>"><%= trimReport.WholeSale %></a></td>
               <td class="title trim"><a onclick="javascript: $('#elementID').removeClass('hideLoader');" href="<%=Url.Content("~/Market/GetKellyBlueBookSummaryAppraisalByTrim?AppraisalId=") %><%=Model.AppraisalId %>&TrimId=<%=trimReport.TrimId %>"><%= trimReport.Retail %></a></td>
           </tr>
		...
		<tr>
			...
			<%
                ...
                {   
                    ...
                    Vincontrol.Web.Models.ExtendedEquipmentOption secondOption=null;
		...
		<tr>
			...
			<%
                ...
                while (addtionalCount < additionalOptionList.Count())
                {
                    ...
                    Vincontrol.Web.Models.ExtendedEquipmentOption secondAdditonalOption = null;
...
<%}
  else
  {
       ...
       <%} %>
         <input class="pad" type="button" name="SaveKBB" value=" Save And Close" id="btnSaveKbbOption" />
...
<script language="javascript">
    ...
    $("#btnSaveKbbOption").click(function(event) {
        ...
        var mileageAdjustment = $("#MileageAdjustment").val();

        $.post('<%= Url.Content("~/Appraisal/SaveKBBOptions") %>', { appraisalId: $("#AppraisalId").val(), OptionSelect: itemoption, TrimId: $("#TrimId").val(), BaseWholeSale: baseWholeSaleNumber, WholeSale: wholeSaleNumber, MileageAdjustment: mileageAdjustment }, function(data) {
    ...
    function SubmitPrintForm(id, trimId) {
        ...
        $.post('<%= Url.Content("~/Appraisal/SaveKBBOptions") %>', { appraisalId: $("#AppraisalId").val(), OptionSelect: itemoption, TrimId: $("#TrimId").val(), BaseWholeSale: baseWholeSaleNumber, WholeSale: wholeSaleNumber, MileageAdjustment: mileageAdjustment }, function(data) {
            if (data == "SessionTimeOut") {
                ...
                var actionUrl = '<%= Url.Action("ViewKBBReportInAppraisal", "MarketReport", new { KBBVehicleId = "PLACEHOLDER",AppraisalId="PLACEHOLDERID",TrimId="PLACEHOLDERTRIMID",PrintOption="PLACEHOLDERPRINTOPTION" } ) %>';

                actionUrl = actionUrl.replace('PLACEHOLDER', id);

                actionUrl = actionUrl.replace('PLACEHOLDERTRIMID', trimId);

                actionUrl = actionUrl.replace('PLACEHOLDERID', $('#AppraisalId').val());
