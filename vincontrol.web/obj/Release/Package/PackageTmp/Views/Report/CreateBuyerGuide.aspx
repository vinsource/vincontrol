<%@ Page Language="C#" Inherits="System.Web.Mvc.ViewPage<Vincontrol.Web.Models.AdminBuyerGuideViewModel>" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            if ($("#IsManufacturerWarranty").val() == "True")
                $("#manufacturerWarranty").html('✔');

            if ($("#IsManufacturerUsedVehicleWarranty").val() == "True")
                $("#manufacturerUsedVehicleWarranty").html('✔');

            if ($("#IsOtherWarranty").val() == "True")
                $("#otherWarranty").html('✔');
            ...
            $("span#btnSave").click(function() {
                $.blockUI({ message: '<div><img src="<%= Url.Content("~/images/ajaxloadingindicator.gif") %>"  /></div>', css: { width: '350px', backgroundColor: 'none', border: 'none'} });
            ...
            $("div#manufacturerWarranty").click(function () {
                var isManufacturerWarranty = $("#IsManufacturerWarranty").val();
                if (isManufacturerWarranty == "" || isManufacturerWarranty == "False") {
                    $("#manufacturerWarranty").html('✔');
                    $("#IsManufacturerWarranty").val('True');
                }
                else {
                    $("#manufacturerWarranty").html('');
                    $("#IsManufacturerWarranty").val('False');
                }
            });

            $("div#manufacturerUsedVehicleWarranty").click(function () {
                var isManufacturerUsedVehicleWarranty = $("#IsManufacturerUsedVehicleWarranty").val();
                if (isManufacturerUsedVehicleWarranty == "" || isManufacturerUsedVehicleWarranty == "False") {
                    $("#manufacturerUsedVehicleWarranty").html('✔');
                    $("#IsManufacturerUsedVehicleWarranty").val('True');
                }
                else {
                    $("#manufacturerUsedVehicleWarranty").html('');
                    $("#IsManufacturerUsedVehicleWarranty").val('False');
                }
            });

            $("div#otherWarranty").click(function () {
                var otherWarranty = $("#IsOtherWarranty").val();
                if (otherWarranty == "" || otherWarranty == "False") {
                    $("#otherWarranty").html('✔');
                    $("#IsOtherWarranty").val('True');
                }
                else {
                    $("#otherWarranty").html('');
                    $("#IsOtherWarranty").val('False');
                }
            });
...
<input type="hidden" name="IsManufacturerUsedVehicleWarranty" id="IsManufacturerUsedVehicleWarranty" value="<%= Model.IsManufacturerUsedVehicleWarranty %>" />
...
<input type="hidden" name="IsLimitedWarranty" id="IsLimitedWarranty" value="<%= Model.IsLimitedWarranty %>" />
    <input type="hidden" name="IsOtherWarranty" id="IsOtherWarranty" value="<%= Model.IsOtherWarranty %>" />
...
<div class="wrapper" id="buyer-guide" style="margin-left:-100px; height: auto !important;">
  ...
  <div class="print-wrap">
    ...
    <div class="vehicle-info">
      <div class="item make">
        <div class="input">        
        <%= Html.TextBoxFor(m => m.Make, new { style = "border-bottom:none; width: 235px;" })%>
        </div>Vehicle Make
      </div>
      <div class="item model">
        <div class="input">
        <%= Html.TextBoxFor(m => m.VehicleModel, new { style = "border-bottom:none; width: 235px;" })%>        
        </div>Model
      </div>
      <div class="item year">
        <div class="input">
        <%= Html.TextBoxFor(m => m.Year, new { style = "border-bottom:none; width: 235px;" })%>
        </div>Year
      </div>
      <div class="item vin">
        <div class="input">
        <%= Html.TextBoxFor(m => m.Vin, new { style = "border-bottom:none; width: 235px;" })%>
        </div>VIN Number
      </div>
      <%--<div class="item stock">
        <div class="input">
        <%= Html.TextBoxFor(m => m.StockNumber, new { style = "border-bottom:none; width: 235px;" })%>
    ...
    <div class="as-is" style="border-bottom: none;">
      ...
      <p style="margin-left: 100px; font-size: 0.4em;">THE DEALER DOES NOT PROVIDE A WARRANTY FOR ANY REPAIRS AFTER SALE.</p>
                <hr style="border-style: dashed; margin-left: 100px;" />
    </div>
    <div class="warranty">
      ...
      <div class="warranty-type" style="margin-top: 20px;">
        <div style="display: inline-block; margin-left: 30px;">
          <div class="sml-checkbox" id="fullWarranty"></div> <p style="font-size: 0.4em;margin-left: 15px;">FULL WARRANTY.</p>  
        </div>
          <div style="display: inline-block; margin-left: 30px;">
        <div class="sml-checkbox" id="limitedWarranty"></div> <p style="margin-top:0px; font-size: 0.4em;margin-left: 15px;">LIMITED WARRANTY - The dealer will pay 
      ...
      <%= Html.TextBoxFor(m => m.PriorRental, new { style = "border-bottom:none; width: 240px; font-size:0.7em; text-transform:uppercase; float: right", placeholder = "Prior rental..." })%>
      </div>
        <div>
                    <h3 class="warranty-header" style="font-size: 0.5em;">NON-DEALER WARRANTIES FOR THIS VEHICLE:</h3>
                    <hr style="border: 4px solid;" />
                    <div class="contract" style="font-size: .4em; display: inline-block; margin-bottom: 10px; width: 100%;">
                        <div style="font-size:1.2em" class="sml-checkbox" id="manufacturerWarranty"></div>
                    ...
                    <div class="contract" style="font-size: .4em; display: inline-block; margin-bottom: 10px; width: 100%;">
                        <div style="font-size:1.2em" class="sml-checkbox" id="manufacturerUsedVehicleWarranty"></div>
                        MANUFACTURER’S USED VEHICLE WARRANTY APPLIES.
                    </div>
                    <div class="contract" style="font-size: .4em; display: inline-block; margin-bottom: 10px; width: 100%;">
                        <div style="font-size:1.2em" class="sml-checkbox" id="otherWarranty"></div>
                        OTHER USED VEHICLE WARRANTY APPLIES.
                    </div>
                    <div class="contract" style="font-size: .4em">
      ...
      <div style="margin-top:0px; font-size:0.4em" class="contract" >
        <div style="font-size:1.2em" class="sml-checkbox" id="serviceContract"></div> 
        ...
        <hr style="border: 4px solid;"/>
            <p style="font-weight: 700; font-size: 30px; display: inline-block;font-size: .4em">ASK THE DEALER IF YOUR MECHANIC CAN INSPECT THE VEHICLE ON OR OFF THE LOT.</p>
                <div style="font-size: 0.4em">
                    <font style="font-weight: 700;">OBTAIN A VEHICLE HISTORY REPORT AND CHECK FOR OPEN SAFETY RECALLS.</font> For information on how to obtain a vehicle history report, visit ftc.gov/usedcars. To check for open safety recalls, visit safercar.gov. You will need the vehicle identification number (VIN) shown above to make the best use of the resources on these sites.
                </div>
                <p style="font-weight: 700; font-size: 30px; display: inline-block;font-size: .4em">SEE OTHER SIDE for important additional information, including a list of major defects that may occur in used motor vehicles.</p>
    </div>
  </div>
</div>
</form>
