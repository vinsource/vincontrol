...
VINControl.GoogleMap.GetZoomLevel = function (dealerLongtitude, dealerLatitude) {
    ...
    var chartZoom = 8;
    if (ChartInfo.fRange.max == 10000) {
        chartZoom = 4;
        chartLongtitude = -97;
        chartLatitude = 38;
    } else if (ChartInfo.fRange.max == 250) {
        chartZoom = 6;
    } else if (ChartInfo.fRange.max == 500) {
        chartZoom = 5;
    }
    return {
...
VINControl.GoogleMap.ConvertHelper = function (map) {
    var convertToMarkerPoints = function (dataResult) {
        ...
        convertToMarkerPoint = function (item, mapData) {
    ...
    getContentDetail = function (title,imageUrl, miles, price, seller, dealerAddress, distance) {
        return "<div>"
                ...
                + "<br>"
                + "Distance: <span >" + distance + " miles</span>"
                + "<br>"
                + "<br>"
                + "</div>";
    };
