...
<script type="text/javascript">
    function ViewDetail(selectedValue) {
        try {
            var arr = selectedValue.split('|');
            $('#divWholeSale').html(arr[0]);
            $('#divMileageAdjustment').html(arr[1]);
            $('#divBaseWholesale').html(arr[2]);
            var urldetail = '<%= Url.Action("GetSingleKarPowerSummary", "Market", new { listingId = ViewData["LISTINGID"], trimId = "_trimID" }) %>';
    ...
    $(document).ready(function () {
        ...
        ViewDetail($("#DDLKBB option:selected").val());
