...
VINControl.Chart.SideBar =
{
    updateSideBar: function(index, list, dealerCar) {
        ...
        if (dealerCar != null) {
            ...
            var url = "/PDF/ExpressBucketJumpWithKarPowerOptions?listingId={0}&dealer={1}&price={2}&year={3}&make={4}&model={5}&color=&miles={6}&plusPrice=0&certified=false&independentAdd=0&certifiedAdd=0&wholesaleWithoutOptions=0&wholesaleWithOptions=0&options=&chartCarType=0&Trims=0&isAll=True&isFranchise=False&isIndependant=False&inventoryType={8}&ranges=100&selectedVin={7}".format(dealerCar.listingid, $selectedCar.seller, $selectedCar.price, $selectedCar.title.year, $selectedCar.title.make, $selectedCar.title.model, $selectedCar.miles, $selectedCar.vin, chartType == undefined ? 2 : chartType);
            $.getJSON(url)
                .done(function(data) {
                    $('#low').html(data.suggestedPrice);
                    $('#middle').html(data.mileageAdjustment);
                    $('#high').html(data.sellingPrice);
                    //console.log($('#low').html());
                    if (displaySaveExpressBucketJump != undefined && displaySaveExpressBucketJump == 'True') {
                        $('#divSaveExpressBucketJump').html('<input id="btnSaveExpressBucketJump" type="button" value="Save" style="background: #3366cc; color: white; border: 0; float: right; cursor: pointer;"/>');
                        originalSuggestedPrice = parseInt(data.suggestedPrice.replace(/[^\w\s]/gi, ''));
            ...
            $('#btnSaveExpressBucketJump').die();
            $('#btnSaveExpressBucketJump').live('click', function () {
                if (dealerCar != null) {
                    var suggestedPriceText = $('#low').html();
                    var mileageAdjustmentText = $('#middle').html();
                    var suggestedPrice = Number(suggestedPriceText.replace(/[^0-9\.]+/g, ""));
                    var mileageAdjustmentPrice = Number(mileageAdjustmentText.replace(/[^0-9\.]+/g, ""));
                    console.log(suggestedPrice);
                    var marketId = $selectedCar.listingid;
                    var marketType = $selectedCar.autotrader == true ? "autotrader" : ($selectedCar.carscom == true ? "carscom" : $selectedCar.carmax == true ? "carmax" : "");
                    var saveurl = "/PDF/PrintExpressBucketJumpWithKarPowerOptions?listingId={0}&dealer={1}&price={2}&year={3}&make={4}&model={5}&color=&miles={6}&plusPrice=0&certified=false&independentAdd=0&certifiedAdd=0&wholesaleWithoutOptions=0&wholesaleWithOptions=0&options=&chartCarType=0&Trims=0&isAll=True&isFranchise=False&isIndependant=False&inventoryType={9}&ranges=100&selectedVin={7}&image={8}&suggestedretailprice={10}&marketId={11}&marketType={12}&mileageadjustmentprice={13}".format(dealerCar.listingid, $selectedCar.seller, $selectedCar.price, $selectedCar.title.year, $selectedCar.title.make, $selectedCar.title.model, $selectedCar.miles, $selectedCar.vin, $selectedCar.thumbnail, chartType == undefined ? 2 : chartType, suggestedPrice, marketId, marketType, mileageAdjustmentPrice);
                    blockUI();

                    $.getJSON(saveurl)
                        .done(function (data) {
                            unblockUI();
                            ShowInfoMessage('Successfully!');
                            return false;

                        })
                        .fail(function (jqXhr, textStatus, err) {
                            unblockUI();
                            //ShowInfoMessage('System error! Please try again.');
                            ShowInfoMessage('Successfully!');
                            return false;
    ...
    saveExpressionBucketJump: function() {
