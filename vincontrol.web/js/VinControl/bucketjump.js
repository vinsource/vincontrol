/// <reference path="../../Views/MassBucketJump/Index.aspx" />
var grid;
var inventoryObj = [];
...
function InventoryGrid(currentViewInfo) {
    var viewInfo = currentViewInfo,
        ...
        appendSortedData = function (data) {
            ...
            $(".vin_listVehicle_holder").html($("#fullSavedBucketJumpTemplate").render(data));
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (result.count == 0) {
                        ...
                        $('#divNoContent').text('There are no vehicles.');
                        $('#divNoContent').show();
...
$(document).ready(function () {
    ...
    $('input[id^=certifiedprice]').live("focus", function () {
        ...
        if (tempCurrentCertifiedprice != tempCertifiedprice) {
            var id = this.id.split('_')[1];
            var saleprice = this.value;
            var pageUrl = '/MassBucketJump/UpdateCertified';
            if (parseInt(saleprice.replace(/,/g, "")) <= 100000000) {
                var img = $(this).parent().find('.imgLoading');
                img.show();

                $.post(pageUrl, { inventoryId: id, certified: $('#massCertified_' + id).is(':checked'), amount: saleprice }, function (data) {
                    img.hide();

                    $.each(inventoryObj, function (index, obj) {
                        if (obj.ListingId == id) {
                            obj.MassBucketJumpCertifiedAmount = saleprice.replace(/,/g, "");
                            return;
                        }
                    });
                });
    ...
    $('input[id^=massCertified]').live('click', function() {
        var id = this.id.split('_')[1];
        if ($(this).is(':checked')) {
            $('#certifiedprice_' + id).attr('disabled', false);
            $.post('/MassBucketJump/UpdateCertified', { inventoryId: id, certified: $(this).is(':checked'), amount: 0 }, function (data) {

                $.each(inventoryObj, function (index, obj) {
                    if (obj.ListingId == id) {
                        obj.MassBucketJumpCertifiedAmount = 0;
                        return;
                    }
                });
            });
        } else {
            $('#certifiedprice_' + id).attr('disabled', true);
            $('#certifiedprice_' + id).val(0);
        }
    });

    var tempACarprice = 0;
    $('input[id^=acarprice]').live("focus", function () {
        ...
        if (tempCurrentCertifiedprice != tempACarprice) {
            var id = this.id.split('_')[1];
            var saleprice = this.value;
            var pageUrl = '/MassBucketJump/UpdateACar';
            if (parseInt(saleprice.replace(/,/g, "")) <= 100000000) {
                var img = $(this).parent().find('.imgLoading');
                img.show();

                $.post(pageUrl, { inventoryId: id, acar: $('#massACar_' + id).is(':checked'), amount: saleprice }, function (data) {
                    img.hide();

                    $.each(inventoryObj, function (index, obj) {
                        if (obj.ListingId == id) {
                            obj.MassBucketJumpACarAmount = saleprice.replace(/,/g, "");
                            return;
                        }
                    });
                });
    ...
    $('input[id^=massACar]').live('click', function () {
        var id = this.id.split('_')[1];
        if ($(this).is(':checked')) {
            $('#acarprice_' + id).attr('disabled', false);
            $.post('/MassBucketJump/UpdateACar', { inventoryId: id, acar: $(this).is(':checked'), amount: 0 }, function (data) {
                
                $.each(inventoryObj, function (index, obj) {
                    if (obj.ListingId == id) {
                        obj.MassBucketJumpACarAmount = 0;
                        return;
                    }
                });
            });
        } else {
            $('#acarprice_' + id).attr('disabled', true);
            $('#acarprice_' + id).val(0);
        }
    });
    $('a[id^=applyMassBucketJump]').live('click', function () {
        var id = this.id.split('_')[1];
        $.fancybox({
            href: '/MassBucketJump/Apply/' + id,
            'type': 'iframe',
            'width': 850,
            'height': 700,
    ...
    $("#submitFilter").click(function (event) {
        ...
        $.ajax({
            ...
            data: {
                ...
                store: $("#selectStore option:selected").val(),
                //fromMile: fromMile,
                //toMile: toMile
            },
            success: function (result) {
...
function initializeFilter(dataList) {
    ...
    dataList.forEach(function (data) {
        ...
        if (arrStore.indexOf(data.Dealer) == -1) {
            arrStore.push(data.Dealer);
        }
    ...
    createListItems(selectStore, arrStore);
