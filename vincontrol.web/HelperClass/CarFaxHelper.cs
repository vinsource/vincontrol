...
{
    public sealed class CarFaxHelperBAK
    {
                    ...
                    if (root["vin-info"] != null)
                    {
                        try
                        {
                            ...
                            if (carfax.PriorRental)
                            {
                                ...
                                var cars =
                                    context.Inventories.Where(i => i.Vehicle.Vin.Equals(Vin));
        ...
        public static CarFaxViewModel ConvertXmlToCarFaxModelAndSave(int vehicleId, string vin, string carFaxUsername, string carFaxPassword)
        {
            var context = new VincontrolEntities();
            var carfax = GetCarFaxReportInDatabase(context, vehicleId);
            ...
            if (!String.IsNullOrEmpty(root.InnerText))
            {
                if (root["vin-info"] != null)
                {
                    try
                    {
                        ...
                        {
                            var cars =
                                context.Inventories.Where(i => i.VehicleId == vehicleId);
                            foreach (var item in cars)
                            {
                                item.PriorRental = true;
                            }
        ...
        public static CarFaxViewModel ConvertXmlToCarFaxModelAndSave(VincontrolEntities context, int vehicleId,string vin, string carFaxUsername, string carFaxPassword)
        {
        ...
        public static CarFaxViewModel GetCarFaxReportInDatabase(string Vin)
        {
            ...
            if (status == 1)
            {
                ...
                var tmp = context.Carfaxes.First(x => x.Vin.Equals(Vin));
                try
                {
                    carfax.Vin = tmp.Vin;
                    carfax.NumberofOwners = tmp.Owner.ToString();
                    carfax.ServiceRecords = tmp.ServiceRecord.GetValueOrDefault();
