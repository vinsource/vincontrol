    ...
    public class SaveCommand : ICommand
    {
        ...
        public void Execute(object parameter)
        {
            if (_vm.NumberOfUploadedFiles == _vm.TotalNumberOfFiles)
            {
                ...
                var r = new Random();
                uriBuilder.Query = string.Format("Function=SaveImageUrl&r={0}&InventoryStatus={1}&ListingId={2}&AppraisalId={3}&User={4}",
                                                 r.Next(), ((App) Application.Current).InventoryStatus,
                                                 ((App)Application.Current).ListingId, ((App)Application.Current).AppraisalId, ((App)Application.Current).User);
